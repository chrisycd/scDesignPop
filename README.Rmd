...
This is an example showing how to run scDesignPop in several sequential steps:


## Step 1: construct a data list
The `eqtlgeno` dataframe consists of eQTL annotations and genotypes for SNPs across
individuals (samples). First a list of components needed to run scDesignPop is required.
This can be done by using the `constructDataPop` function.
...
Next, a marginal model is specified and fit using the `fitMarginalPop` function.
```{r}
